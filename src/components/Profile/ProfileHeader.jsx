...
const ProfileHeader = () => {
  ...
  const { isLoading, isFollowing, handleFollow } = useFollow(userProfile?.uid);

  return (
    <Flex gap={{ base: 4, sm: 10 }} py={10} direction={{ base: "column", sm: "row" }}>
      <AvatarGroup size={{ base: "xl", md: "2xl" }} justifySelf={"center"} alignSelf={"flex-start"} mx={"auto"}>
        <Avatar name={userProfile.username} src={userProfile.profilePictureURL} alt={userProfile.username} />
      </AvatarGroup>
      <VStack alignItems={"start"} gap={2} mx={"auto"} flex={1}>
        <Flex gap={4} direction={{ base: "column", sm: "row" }} justifyContent={{ base: "center", sm: "flex-start" }} alignItems={"center"} w={"full"}>
          ...
          {authUser && authUser.username === userProfile.username && (
            ...
            {authUser && authUser.username !== userProfile.username && (
            <Flex gap={4} alignItems={"center"} justifyContent={"center"}>
              <Button bg={"blue.500"} color={"white"} _hover={{ bg: "blue.600" }} size={{ base: "xs", md: "sm" }} onClick={handleFollow} isLoading={isLoading} >
